#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Manche D Montchat - backup + safety diff KV, AUCUN POST.

Miroir de scripts/_b2a_backup_diff_montchat.py pour la manche D
(secteur_assignments:dauphine-lacassagne-montchat). Pose D = +332 tags as.type
mono/copro_non_immat (hors-RNC), AJOUTES par-dessus les 103 B2a.

Le candidat (data/_kv_assign_montchat.D.candidate.json) est l'OBJET POST COMPLET
(435 entrees = 103 B2a + 332 D) genere par scripts/_D_derive_montchat.py. Ce
script :
  A. GET prod -> data/_kv_assign_montchat.D.preaudit.bak (GET gracieux 404 -> {}).
  B. confirme qu'AUCUN des 332 nouveaux tags n'ecrase un tag existant DIFFERENT
     (no-op identique tolere ; valeur DIFFERENTE : STOP).
  C. SAFETY DIFF candidat vs prod : 0 retrait, 0 modif de cle non-D, ajouts/no-op
     uniquement -> attendu +332/0/0 (KV 103 -> 435).
...
BACKUP = ROOT / "data" / "_kv_assign_montchat.D.preaudit.bak"
CANDIDATE = ROOT / "data" / "_kv_assign_montchat.D.candidate.json"

ALLOWED_TYPES = {"bureaux", "mono", "copro_non_immat"}
# Les NOUVEAUX tags (manche D) sont uniquement mono | copro_non_immat.
ALLOWED_NEW_TYPES = {"mono", "copro_non_immat"}
...
def main():
    ...
    if not CANDIDATE.exists():
        abort(f"candidat absent : {CANDIDATE.name} "
              "(relancer scripts\\_D_derive_montchat.py).")
    ...
    for k, v in cand_assign.items():
        if (v or {}).get("type") not in ALLOWED_TYPES:
            abort(f"candidat : tag inattendu {k} -> {v} "
                  "(attendu bureaux|mono|copro_non_immat).")

    print("=" * 64)
    print("Manche D Montchat - BACKUP + SAFETY DIFF (candidat complet, aucun POST)")
    print("=" * 64)
    ...
    for k, v in cand_assign.items():
        ...
        if cur is None:
            ...
            if v.get("type") not in ALLOWED_NEW_TYPES:
                abort(f"ajout {k} type {v.get('type')} non autorise en manche D "
                      "(attendu mono|copro_non_immat).")
        elif cur == v.get("type"):
            noops.append(k)
        else:
    ...
    print("=" * 64)
    print("  STOP. Aucun POST. Valider avant _D_post_montchat.py.")
