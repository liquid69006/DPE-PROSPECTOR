#!/usr/bin/env python3
"""Reclassement KV DL : PATCH 5 cles copro_non_immat -> social.

Issu du cross-check MAJIC approfondi des 9 cibles ayant Metropole de Lyon
ou Commune Lyon a 50%+ (parmi les 102 copro_non_immat residuelles bdnb>=3).

Verdict par adresse :
  - 4 SOCIAL pur (100% public/HLM cumul) : Metropole + bailleur HLM 50/50,
    sans aucune part privee.
  - 1 SOCIAL pur via admin publique (Direction Penitentiaire + ETAT) :
    100% public, residentiel collectif (logement de fonction).
  - 4 MIXTES (laisses copro_non_immat) : CNP/AEW SCPI privees, CDC+
    societes privees 44%, AESTIAM 35%.

5 cibles reclassees ici (SOCIAL pur, decision haute confiance) :
  5 PLACE FERRANDIERE       Grand Lyon Habitat OPH 50% + Fondation Aralis 50%
  36 RUE STE ANNE BARABAN   Direction Penitentiaire 50% + ETAT 50% (logt fonction)
  253 RUE PAUL BERT         Metropole 50% + Grand Lyon Habitat OPH 50%
  9 RUE PETITES SOEURS      Metropole 50% + ADOMA 50%
  232 RUE PAUL BERT         Metropole 50% + IRA HLM 50%

4 cibles non reclassees (vraies copros mixtes) :
  27 + 23 LOUIS JASSERON   CNP + AEW + AEW CILOGER (90%+ prive)
  74B MAURICE FLANDIN      CDC Habitat 54% + 4 societes privees 44%
  23 ST EUSEBE             OPH Metropole 65% + AESTIAM 35% prive

Pattern script identique a _fix_reclass_3_dl.py.

Usage :
  python scripts/_fix_reclass_5_social_dl.py <JWT_TOKEN>
  ou : DPE_TOKEN=<jwt> python scripts/_fix_reclass_5_social_dl.py
"""
...
BAK      = KV_LOCAL.with_suffix(KV_LOCAL.suffix + ".prereclass5social.bak")
...
CIBLES = [
    ("5|PLACE|FERRANDIERE",          "social"),
    ("36|RUE|STE ANNE DE BARABAN",   "social"),
    ("253|RUE|PAUL BERT",            "social"),
    ("9|RUE|PETITES SOEURS",         "social"),
    ("232|RUE|PAUL BERT",            "social"),
]
...
print("=" * 90)
print(f"KV DL : PATCH {len(CIBLES)} cles copro_non_immat -> social (cross-check MAJIC 50%+ public)")
print("=" * 90)
...
for cle, target in CIBLES:
    ...
    tag = (cur or {}).get("type")
    if not cur:
        print(f"    {cle:<35} -> SKIP (absent du KV)")
        skip.append((cle, "absente"))
    elif tag == target:
        print(f"    {cle:<35} -> SKIP (deja type={target}, idempotent)")
        skip.append((cle, f"deja-{target}"))
    elif tag != "copro_non_immat":
        print(f"    {cle:<35} -> SKIP (tag actuel = {tag!r}, attendu copro_non_immat)")
        skip.append((cle, f"tag-{tag}"))
    else:
        extras = sorted(k for k in cur if k != "type")
        print(f"    {cle:<35} -> PATCH copro_non_immat -> {target:<8}  (preserve {extras})")
...
for cle, target in to_patch:
    ...
    if not after:
        print(f"    {cle:<35} -> KO (disparue)"); errs.append(cle); continue
    if after.get("type") != target:
        print(f"    {cle:<35} -> KO (type={after.get('type')!r}, attendu {target})"); errs.append(cle); continue
    extras_before = {k: v for k, v in before.items() if k != "type"}
    extras_after  = {k: v for k, v in after.items()  if k != "type"}
    if extras_before == extras_after:
        print(f"    {cle:<35} -> OK type={target:<8}  ({len(extras_after)} champ(s) preserve(s))")
    else:
        print(f"    {cle:<35} -> WARN type={target} mais extras differents")
