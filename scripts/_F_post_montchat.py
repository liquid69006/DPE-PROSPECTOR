#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Manche F Montchat - PATCH KV : POST du candidat (381 + social/mixte F).

Calque de scripts/_D_post_montchat.py pour secteur_assignments:
...
Sequence :
  ...
  3. VERIF : re-GET prod ; confirme prod == candidat.
...
def main():
    ...
    if not BACKUP.exists():
        abort(f"backup absent : {BACKUP.name} (relancer _F_backup_diff_montchat.py).")
    ...
    print("=" * 64)
    print("Manche F Montchat - PATCH KV (POST candidat complet)")
    print("=" * 64)

    # sanity candidat = backup + uniquement ajouts/no-op (social|mixte) ----
    ...
    n_soc = sum(1 for k in added if ca[k].get("type") == "social")
    n_mix = sum(1 for k in added if ca[k].get("type") == "mixte")
    print(f"  candidat local OK : +{len(added)} ajouts "
          f"({n_soc} social + {n_mix} mixte), 0 retrait/modif.")
    ...
    print(f"     verif OK : prod == candidat (assignments={len(np2['assignments'])}, "
          f"dont +{len(added)} F). Reste inchange.")
