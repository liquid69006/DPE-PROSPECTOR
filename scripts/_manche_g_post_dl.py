#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Manche G - PATCH KV : POST du candidat (re-tags cible_0vente via levier BDNB).

Calque EXACT de _manche_f_post_dl.py. PUR re-tag de type. Un seul POST.
Endpoint manche D/F (/secteur-assignments/<agence>).

A lancer dans une session PowerShell ou DPE_JWT est charge :
    . scripts\\load_jwt.ps1
    python scripts\\_manche_g_post_dl.py
"""
...
def main():
    ...
    print("=" * 70)
    print("MANCHE G - PATCH KV (POST candidat, re-tags cible_0vente) - DL")
    print("=" * 70)
    ...
    if not changed:
        abort("local : 0 modif (candidat == backup ?).")
    EXPECT_TO = {}
    for k in changed:
    ...
    print(f"  sanity local OK : {len(changed)} re-tags type-only (tous cible_0vente_*).")
    ...
    print(f"     verif OK : {len(EXPECT_TO)} re-tags a leur cible ; count={len(p2a)} "
          "inchange ; fusions/noms inchanges.")
    ...
    print("=" * 70)
    print("  OK. re-tags appliques et verifies en prod. Commit du miroir = "
          "manche suivante.")
