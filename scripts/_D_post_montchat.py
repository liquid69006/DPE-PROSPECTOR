#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Manche D Montchat - PATCH KV : POST du candidat (435 = 103 B2a + 332 D).

Miroir de scripts/_b2a_post_montchat.py pour secteur_assignments:
dauphine-lacassagne-montchat. Un seul POST de l'objet candidat complet (le worker
ecrase tout l'objet assignments).

A lancer dans une session PowerShell ou DPE_JWT est charge, APRES
_D_backup_diff_montchat.py :
...
Sequence :
  ...
  3. VERIF : re-GET prod ; confirme prod == candidat. KO -> alerte, miroir intact.
...
def main():
    ...
    if not BACKUP.exists():
        abort(f"backup absent : {BACKUP.name} (relancer _D_backup_diff_montchat.py).")
    ...
    print("=" * 64)
    print("Manche D Montchat - PATCH KV (POST candidat complet)")
    print("=" * 64)

    # sanity candidat = backup + uniquement ajouts/no-op (mono|copro_non_immat) ----
    ...
    for k in added:
        if (ca.get(k) or {}).get("type") not in ALLOWED_NEW_TYPES:
    ...
    n_copro = sum(1 for k in added if ca[k].get("type") == "copro_non_immat")
    print(f"  candidat local OK : +{len(added)} ajouts "
          f"({n_mono} mono + {n_copro} copro_non_immat), 0 retrait/modif.")
    ...
    print(f"     verif OK : prod == candidat (assignments={len(np2['assignments'])}, "
          f"dont +{len(added)} D). Reste inchange.")
